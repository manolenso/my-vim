" gulp - A plugin to help working with Gulp
" Maintainer: mklabs

if exists("g:loaded_gulp") || v:version < 700 || &cp
  finish
endif
...
"
" todo:
"   - should probably do the gulp detect only on buff enter, instead of
...
"   like :GR<cmd>, or :Gulp<cmd>.  The fugitive family of :G<cmd> with
"   gulp ones may be confusing in tab completions.
"
...
function! s:Gulp(bang, args)
  let cmd = 'gulp --no-color '.a:args
  execute ':!'.cmd
endfunction
...
" the gulp source file (resolved globally or locally)
function! s:InitFind()
...
" gulp here, with lint or test task), an three splitting patterns. One
...
" ### Gulpdoc
"
" Task command -> :Gulpdoc <page>
"
" Open a given gulp doc page in default browser
" borrowed to vim-rails
"
function! s:GulpDoc(bang, page)
  let url = 'https://www.npmjs.org/package/'.a:page.'.md'
  echo '... Opening ' . url . ' ...'
  if exists(':OpenURL')
    exe 'OpenURL '.url
  else
...
" Task command -> :Glint
"
" gulp lint wrapper, collect output for quickfix window. You can then
...
"
" Same as :Glint, runs `gulp test` task, collect output and display any
...
"
" todo: gulp test seems to always have coloured output. open issue.
"
function! s:GTest(bang, filename)
  ...
  let script = join([s:dirname, 'scripts', 'gulp-help.js'], '/')
  let entries = s:NodeQuickFix(system('gulp --no-color test'), script)
  ...
  for entry in entries
    ...
    for error in errors
      ...
      " find file by globing the task dir, gulp output only include basename
...
" node variant quickfix helper, mainly for parsing gulp test output
...
" detect a gulp project, a little crude right now, just checking cwd's
" gulpfile exists todo: it shoulld detect on both cwd (when starting
" vim from console) and actual vim path (eg. vim ./some/nested/path) to
" handle both case
"
function! s:Detect()
  let gulpfile = join([s:cwd, 'gulp.js'], '/')
  if filereadable(gulpfile)
    return s:GulpInit()
  endif

  let gulpfile = join([s:cwd, 'Gulpfile.js'], '/')
  if filereadable(gulpfile)
    return s:GulpInit()
  endif

  let gulpfile = join([s:cwd, 'gulp.coffee'], '/')
  if filereadable(gulpfile)
    return s:GulpInit()
  endif

  let gulpfile = join([s:cwd, 'Gulpfile.coffee'], '/')
  if filereadable(gulpfile)
    return s:GulpInit()
  endif
endfunction

" redefine gulplint autocmd group
...
function! s:GulpInit()
  ...
  call s:initOpenURL()
  " gulp-project commands
  call s:GulpCommands()
  " gulp-project specific gf extension
  call s:InitFind()
endfunction

" define commands loaded only on GulpDetect
function! s:GulpCommands()
  ...
  " should complete lint and test for subtargets, probably by parsing the gulpfile =/
...
command! -bar -bang -nargs=* Gulp call s:Gulp(<bang>0,<q-args>)
